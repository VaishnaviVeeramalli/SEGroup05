...
router.get('/auth', auth, (req,res)=>{ 
    ...
    if(usertype=="Student") {
        res.status(200).json({ 
            "_id":req.student._id,
            "isAdmin": req.student.role === 0 ? false : true,
            "isAuth":true,
            "email":req.student.email,
            "name":req.student.name,
            "role":req.student.role,
            "cart":req.student.cart,
            "history":req.student.history,
            "usertype":usertype
        })
    } else if (usertype=="Club"){
        res.status(200).json({ 
            "_id":req.club._id,
            "isAdmin": req.club.role === 0 ? false : true,
            "isAuth":true,
            "email":req.club.email,
            "name":req.club.name,
            "role":req.club.role,
            "cart":req.club.cart,
            "history":req.club.history,
            "usertype":usertype
        })
    } else{
        res.status(200).json({ 
...
router.post('/addToCart',auth,(req,res)=>{
    ...
    console.log("user type in add to cart ", usertype)
    if(usertype == "Student") {
        const userId = req.student._id;
        if(process.env.NODE_ENV != 'test') {
            productId =ObjectId.createFromHexString(req.body.productId);
        } 
        Student.findOne({_id:userId},(err,user)=>{
        ...
        if(duplicate===true){
            console.log('duplicate');
            Student.findOneAndUpdate({_id:userId,"cart.id":productId },
            { $inc: { "cart.$.quantity": 1 } },
            { new: true },
            (err, userInfo) => {
        ...
        const userId = req.club._id;
        if(process.env.NODE_ENV != 'test') {
            productId =ObjectId.createFromHexString(req.body.productId);
        } 
        Club.findOne({_id:userId},(err,user)=>{
        ...
        if(duplicate===true){
            console.log('duplicate');
            Club.findOneAndUpdate({_id:userId,"cart.id":productId },
            { $inc: { "cart.$.quantity": 1 } },
            { new: true },
            (err, userInfo) => {
                if (err) return res.json({ success: false, err });
...
router.post('/removeCartItem',auth,(req,res)=>{ 
    ...
    Student.findOneAndUpdate({"_id":userId},{$pull:{cart:{id:Id}}}, { new: true },(err,userInfo)=>{ 
    ...
    Club.findOneAndUpdate({"_id":userId},{$pull:{cart:{id:Id}}}, { new: true },(err,userInfo)=>{ 
        let cart = userInfo.cart;
        // console.log(cart)
        let array = cart.map(item => {
            return item.id
...
router.get('/getHistory',auth,(req,res)=>{
    ...
    if(req.cookies.usertype=="Student") {
        Student.findOne({_id:req.student._id},(err,docs)=>{
            if(err){
                return res.json({
                    success:false,
                    err
        ...
        Club.findOne({_id:req.club._id},(err,docs)=>{
            if(err){
                return res.json({
                    success:false,
                    err
