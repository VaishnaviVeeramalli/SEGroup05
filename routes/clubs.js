...
router.get('/auth', auth, (req,res)=>{ 
    ...
    res.status(200).json({
        ...
        "name":req.club.name,
        "registrationnumber":req.club.registrationnumber,
        "phone": req.club.phone,
        "role":req.user.role,
        "cart":req.user.cart,
...
router.post('/login',(req,res)=>{
    ...
    Club.findOne({email:req.body.email},(err,club)=>{
        if(!club){
            return res.json({
                loginSuccess:false,
        ...
        club.comparePassword(req.body.password,(err,isMatch)=>{
            if(err || !isMatch){
                return res.json({
                    loginSuccess:false,
            ...
            club.generateToken((err,user)=>{
                ...
                res.cookie("x_authExp", club.tokenExp);
                return res.cookie('x_auth', club.token)
                .status(200)
                .json({
                    loginSuccess:true,
                    _id:club.id,
